...
def extract_condition(df, name, cue_val, probe_col, keep_X):
    ...
    if sub.empty:
        # no trials of this type at all
        return pd.DataFrame()

    # get all trial IDs for this condition (even if count would be zero)
    trial_ids = sub['NewTrialId'].unique()

    # contiguous gaze intervals
    sub['interval'] = sub['AOI'].ne(sub['AOI'].shift()).cumsum()
    sub['count']    = sub.groupby(['AOI','interval']).cumcount() + 1

    # compute non-zero counts per trial as before
    counts = (
        sub
        .groupby('interval').last()
        .query('AOI == 2')
        .query('ACC == 1')
        .groupby('NewTrialId')['count']
        .sum()
        .reset_index(name='count')
    )

    # now build a full table including zeros
    full = pd.DataFrame({'NewTrialId': trial_ids})
    full = full.merge(counts, on='NewTrialId', how='left').fillna({'count': 0})
    full['count'] = full['count'].astype(int)

    # convert count → tvd
    full['tvd_within_trial'] = full['count'] * TVD_FACTOR

    # label & return
    full.insert(0, 'Subject', df['Subject'].iat[0])
    full['condition'] = name
    return full
...
outfile = os.path.join(output_path, 'Bottom_TVD_Within_Correct_Trials.xlsx')
