# CORRECT TRIALS ONLY nonxlsx program
...
# FILE IO PATHS!
csv_input_path = './!INPUT//'  # where our gazedata files will be put into!
...
# get all csv (gazedata) files and store into a list
csv_files = glob.glob(os.path.join(csv_input_path, "*.gazedata"))
...
for file in csv_files:
    ...
    # read csv file
    df = pd.read_csv(file, delimiter="\t")  # delimiter indicates gazedata has tab seperated values

    # Get relevant columns from df
    df_2 = df[['Subject', 'TrialId', 'Cue', 'AOIProbe1', 'AOIProbe2', 'AOICue', 'AOI', 'AOIStimulus', 'ACC', 'RT',
               'CurrentObject']]
    ...
    block_df = pd.DataFrame(df_2[['AOI', 'AOICue', 'ACC', 'CurrentObject', 'AOIProbe1', 'AOIProbe2', 'TrialId']])
    ...
    print("\nBegin AOI Visit Count analysis...")  # DEBUG LINE
    ...
    # get relevant columns
    count_aoi = pd.DataFrame(df_2[['AOI', 'AOICue', 'ACC', 'CurrentObject', 'AOIProbe1', 'AOIProbe2', 'TrialId']])
    ...
    tv = df_2[['AOI', 'AOICue', 'ACC', 'CurrentObject', 'AOIProbe1', 'AOIProbe2', 'TrialId']]
    ...
    AX_df_correct_block_1 = block_1[(block_1['AOICue'] == 'A') &
               (block_1['AOIProbe1'] == 'X') &
               (block_1['CurrentObject'] == 'ISI') &
               (block_1['ACC'] == 1)].reset_index(drop=True)
    ...
    AX_df_correct_block_2 = block_2[(block_2['AOICue'] == 'A') &
                                    (block_2['AOIProbe1'] == 'X') &
                                    (block_2['CurrentObject'] == 'ISI') &
                                    (block_2['ACC'] == 1)].reset_index(drop=True)
    ...
    AX_df_correct_block_3 = block_3[(block_3['AOICue'] == 'A') &
                                    (block_3['AOIProbe1'] == 'X') &
                                    (block_3['CurrentObject'] == 'ISI') &
                                    (block_3['ACC'] == 1)].reset_index(drop=True)
    ...
    AX_df_correct_block_4 = block_4[(block_4['AOICue'] == 'A') &
                                    (block_4['AOIProbe1'] == 'X') &
                                    (block_4['CurrentObject'] == 'ISI') &
                                    (block_4['ACC'] == 1)].reset_index(drop=True)
    ...
    AX_df = df_2_ff[(df_2_ff['AOICue'] == 'A') &
                    (df_2_ff['AOIProbe1'] == 'X') &
                    (df_2_ff['CurrentObject'] == 'ISI') &
                    (df_2_ff['ACC'] == 1)].reset_index(drop=True)
    ...
    def get_first_fixation_proportions(df_list):
        ...
        try:
            ...
            prop_top_b4_bot = 0  # prevents divide by zero error
        try:
            prop_bot_b4_top = num_bot_b4_top / len(df_list)
        except:
            prop_bot_b4_top = 0  # prevents divide by zero error
...
output.to_excel(f'{output_path}' + '//' + f'AX_CPT_CORRECT_TRIALS_ONLY_MAIN_OUTPUT.xlsx')
