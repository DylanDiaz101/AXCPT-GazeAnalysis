# CORRECT TRIALS ONLY XLSX Windows
...
for filepath in glob.glob(excel_input_path+'*.xlsx'):
    ...
    call = ["python", "./xlsx2csv/xlsx2csv.py", filepath, csv_input_path+'{}.csv'.format(filename.group(2))]
...
for i, return_code in enumerate(pool.imap(functools.partial(subprocess.call, shell=True), commands)):
    if return_code != 0:
...
output.to_excel(f'{output_path}' + '//' + f'AXCPT_CORRECT_TRIALS_ONLY_MAIN_OUTPUT.xlsx')
